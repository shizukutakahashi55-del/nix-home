# ❄️ NixOS Configuration (Flakes) + NVIDIA + Home Manager

Modular, declarative **NixOS** configuration, managed with **Nix Flakes**, **Git**, **Home Manager**, and **KDE Plasma**, on an **NVIDIA** GPU.
...
* **Stays a NixOS system module** (`modules/programs/`) when it needs something only the system can give it: a driver library path (`obs.nix`, needs the NVIDIA `LD_LIBRARY_PATH`), a setuid wrapper (`steam.nix`), a nixpkgs override built at the system level (`suwayomi.nix`), or a plugin a **system-level** desktop session has to discover (`kde.nix`'s KWin blur effect and Kvantum style — Plasma 6 itself, `services.desktopManager.plasma6.enable`, is also inherently system-level). `hyprland.nix` stays as-is too, since the compositor session and its user config are handled entirely by `dotfiles-nix`.
* **Moves to Home Manager** (`home/programs/`) otherwise: it's just a regular application with no system integration — browsers, chat/communication apps, dev tooling, gaming utilities, media players, terminal/CLI tools.

If you add a new plain app later, put it in `home/programs/`, not `modules/programs/`.
